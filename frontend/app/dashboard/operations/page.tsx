...
function severityBadge(severity: AlertSeverity) {
  const map: Record<AlertSeverity, { bg: string; text: string; dot: string }> = {
    ...
    warning: { bg: "bg-amber-400/10 border-amber-400/20", text: "text-amber-400", dot: "bg-amber-400" },
...
function incidentSeverityBadge(severity: IncidentSeverity) {
  const map: Record<IncidentSeverity, string> = {
    ...
    P3: "bg-amber-400/10 border-amber-400/20 text-amber-400",
...
function statusBadge(status: IncidentStatus) {
  const map: Record<IncidentStatus, { bg: string; text: string; dot: string }> = {
    ...
    investigating: { bg: "bg-amber-400/10 border-amber-400/20", text: "text-amber-400", dot: "bg-amber-400 animate-pulse" },
...
function ProgressBar({ value, max, color }: { value: number; max: number; color: string }) {
  ...
  const barColor =
    pct > 80 ? "bg-red-400" : pct > 60 ? "bg-amber-400" : color;
  return (
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-amber-400/10 border border-amber-400/20">
                <Activity size={14} className="text-amber-300" />
              </div>
              <div>
                <h1 className="text-2xl font-bold text-amber-50 font-display">Operations</h1>
      ...
      <section className="rounded-3xl border border-white/10 bg-black/60 p-6 backdrop-blur-2xl">
        {/* Tab Navigation */}
        <div className="flex items-center gap-1 border-b border-white/5 pb-4 mb-6">
          {[
          ...
          <div className="space-y-3 animate-in fade-in duration-300">
            ...
            {ACTIVE_ALERTS.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-emerald-400/20 bg-emerald-400/5 p-8 text-center">
                ...
                <p className="text-[10px] text-amber-100/30 mt-1">All metrics within thresholds</p>
              </div>
            ) : (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/5">
                      <th className="p-3 text-left text-[10px] uppercase tracking-widest text-amber-100/40 font-semibold">ID</th>
                      <th className="p-3 text-left text-[10px] uppercase tracking-widest text-amber-100/40 font-semibold">Severity</th>
                      <th className="p-3 text-left text-[10px] uppercase tracking-widest text-amber-100/40 font-semibold">Metric</th>
                      <th className="p-3 text-left text-[10px] uppercase tracking-widest text-amber-100/40 font-semibold">Threshold</th>
                      <th className="p-3 text-left text-[10px] uppercase tracking-widest text-amber-100/40 font-semibold">Current</th>
                      <th className="p-3 text-left text-[10px] uppercase tracking-widest text-amber-100/40 font-semibold">Fired</th>
                    </tr>
                  </thead>
                  <tbody>
                    {ACTIVE_ALERTS.map((alert) => (
                      <tr key={alert.id} className="border-t border-white/5 hover:bg-white/[0.02] transition-colors">
                        <td className="p-3">
                          <code className="text-[10px] font-mono text-amber-200/60">{alert.id}</code>
                        </td>
                        <td className="p-3">{severityBadge(alert.severity)}</td>
                        <td className="p-3 text-xs font-medium text-amber-50">{alert.metric}</td>
                        <td className="p-3">
                          <code className="rounded bg-black/60 border border-white/5 px-2 py-0.5 text-[10px] font-mono text-amber-100/50">
                            {alert.threshold}
                          </code>
                        </td>
                        <td className="p-3">
                          <code className={`rounded bg-black/60 border border-white/5 px-2 py-0.5 text-[10px] font-mono ${
                            alert.severity === "critical" ? "text-red-400" : alert.severity === "warning" ? "text-amber-400" : "text-sky-400"
                          }`}>
                            {alert.current}
                          </code>
                        </td>
                        <td className="p-3 text-[10px] text-amber-100/40">{formatTimestamp(alert.fired)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/5">
                    <th className="p-3 text-left text-[10px] uppercase tracking-widest text-amber-100/40 font-semibold">ID</th>
                    <th className="p-3 text-left text-[10px] uppercase tracking-widest text-amber-100/40 font-semibold">Severity</th>
                    <th className="p-3 text-left text-[10px] uppercase tracking-widest text-amber-100/40 font-semibold">Title</th>
                    <th className="p-3 text-left text-[10px] uppercase tracking-widest text-amber-100/40 font-semibold">Status</th>
                    <th className="p-3 text-left text-[10px] uppercase tracking-widest text-amber-100/40 font-semibold">Created</th>
                  </tr>
                </thead>
                <tbody>
                  {INCIDENTS.map((inc) => (
                    <tr key={inc.id} className="border-t border-white/5 hover:bg-white/[0.02] transition-colors">
                      <td className="p-3">
                        <code className="text-[10px] font-mono text-amber-200/60">{inc.id}</code>
                      </td>
                      <td className="p-3">{incidentSeverityBadge(inc.severity)}</td>
                      <td className="p-3 text-xs font-medium text-amber-50">{inc.title}</td>
                      <td className="p-3">{statusBadge(inc.status)}</td>
                      <td className="p-3 text-[10px] text-amber-100/40">{formatTimestamp(inc.created)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="space-y-3 animate-in fade-in duration-300">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/5">
                    <th className="p-3 text-left text-[10px] uppercase tracking-widest text-amber-100/40 font-semibold">Correlation ID</th>
                    <th className="p-3 text-left text-[10px] uppercase tracking-widest text-amber-100/40 font-semibold">Method</th>
                    <th className="p-3 text-left text-[10px] uppercase tracking-widest text-amber-100/40 font-semibold">Path</th>
                    <th className="p-3 text-center text-[10px] uppercase tracking-widest text-amber-100/40 font-semibold">Status</th>
                    <th className="p-3 text-right text-[10px] uppercase tracking-widest text-amber-100/40 font-semibold">Duration</th>
                  </tr>
                </thead>
                <tbody>
                  {CORRELATION_TRACES.map((trace) => (
                    <tr key={trace.correlationId} className="border-t border-white/5 hover:bg-white/[0.02] transition-colors">
                      <td className="p-3">
                      ...
                      <td className="p-3">
                        <code className="text-xs font-mono text-amber-100/60">{trace.path}</code>
                      </td>
                      ...
                      <td className="p-3 text-right">
                        <span className={`text-xs font-mono ${
                          parseInt(trace.duration) > 2000 ? "text-red-400" : parseInt(trace.duration) > 500 ? "text-amber-400" : "text-emerald-400"
                        }`}>
                          {trace.duration}
                        </span>
                      </td>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <section className="rounded-3xl border border-white/10 bg-black/60 p-6 backdrop-blur-2xl">
          <div className="flex items-center gap-2 mb-1">
            <Server size={14} className="text-amber-200" />
            <h2 className="text-sm font-semibold uppercase tracking-widest text-amber-200/90 font-display">
              System Health
            </h2>
          </div>
          <p className="text-[0.65rem] text-amber-100/40 uppercase tracking-wider mb-5">
            ...
            <div className="rounded-2xl border border-white/5 bg-black/40 p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-amber-50">{SYSTEM_HEALTH.cpu.label}</span>
                </div>
              ...
              <p className="text-[9px] text-amber-100/30 mt-1.5">4 vCPU cores available</p>
            </div>
            ...
            <div className="rounded-2xl border border-white/5 bg-black/40 p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-amber-50">{SYSTEM_HEALTH.memory.label}</span>
                </div>
                <span className="text-sm font-bold text-emerald-400">
              ...
              <p className="text-[9px] text-amber-100/30 mt-1.5">
            ...
            <div className="rounded-2xl border border-white/5 bg-black/40 p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-amber-50">{SYSTEM_HEALTH.disk.label}</span>
                </div>
                <span className="text-sm font-bold text-emerald-400">
            ...
            <div className="rounded-2xl border border-white/5 bg-black/40 p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-amber-50">{SYSTEM_HEALTH.network.label}</span>
                </div>
              ...
              <p className="text-[9px] text-amber-100/30 mt-2">Latency: 1.2ms (internal), 24ms (edge)</p>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-3xl border border-white/10 bg-black/60 p-6 backdrop-blur-2xl">
          <div className="flex items-center gap-2 mb-1">
            <HardDrive size={14} className="text-amber-200" />
            <h2 className="text-sm font-semibold uppercase tracking-widest text-amber-200/90 font-display">
              Backup Status
            </h2>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-emerald-400/10 bg-emerald-400/5 p-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-amber-100/40 mb-1">Completed</p>
                  <p className="text-sm font-medium text-amber-50">{formatTimestamp(BACKUP_STATUS.lastBackup)}</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-amber-100/40 mb-1">Size</p>
                  <p className="text-sm font-medium text-amber-50">{BACKUP_STATUS.backupSize}</p>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-white/5 bg-black/40 p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-semibold text-amber-200">Next Scheduled</span>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-amber-100/40 mb-1">Scheduled</p>
                  <p className="text-sm font-medium text-amber-50">{formatTimestamp(BACKUP_STATUS.nextScheduled)}</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-amber-100/40 mb-1">Retention</p>
                  <p className="text-sm font-medium text-amber-50">{BACKUP_STATUS.retention}</p>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-white/5 bg-black/40 p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-amber-100/40 mb-1">Integrity Check</p>
                  <p className="text-xs text-amber-100/50">SHA-256 checksum verification on latest snapshot</p>
                </div>
            ...
            <div className="rounded-2xl border border-dashed border-amber-300/10 bg-amber-300/[0.02] p-4">
              <p className="text-[10px] text-amber-100/40 leading-relaxed">
                Automated daily backups run at <code className="text-amber-200/60">04:00 UTC</code> via cron.
                ...
                Restore procedure documented in <code className="text-amber-200/60">infra/runbooks/backup-restore.md</code>.
              </p>
            </div>
          </div>
        </section>
      ...
      <section className="rounded-3xl border border-white/10 bg-black/60 p-6 backdrop-blur-2xl">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <div key={p.pillar} className="rounded-2xl border border-white/5 bg-black/40 p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="rounded-full bg-amber-400/10 border border-amber-300/20 px-2 py-0.5 text-[9px] font-bold text-amber-300 uppercase tracking-wider">
                  {p.pillar}
                </span>
              ...
              <p className="text-xs font-semibold text-amber-50">{p.name}</p>
              <p className="text-[10px] text-amber-100/40 mt-0.5">{p.detail}</p>
            </div>
          ))}
        </div>
      </section>
