# ChickenHawk Rebuild Specification
...
ChickenHawk is the A.I.M.S. execution engine — the throughput regulator between Boomer_Ang supervision and Lil_Hawk atomic workers. It converts high-level deployment plans into deterministic execution packets, enforcing policy gates at every step.

### Command Chain

```
User → ACHEEVY → Boomer_Angs → ChickenHawk → Squad → Lil_Hawk
```

### Core Principle

**"Packet first. Proof always."** — ChickenHawk won't start a run until every gate is green, and won't mark a run done until every proof is collected.
...
- **Owner-only:** ChickenHawk runs on owner's VPS, not shared infrastructure
...
Circuit Box becomes ChickenHawk's control plane. Policy levers:
...
User speaks → Browser MediaRecorder
  ...
  → ChickenHawk manifest (same as text flow)
...
- ChickenHawk runs exclusively on the owner's VPS (VPS 2)
...
*Generated by A.I.M.S. ChickenHawk Rebuild Specification — 2026-02-13*
